...
class RealTimeStereo(threading.Thread):
    ...
    def _setup_gui(self):
        ...
        cv2.namedWindow(self._camera_window_name_left, cv2.WINDOW_AUTOSIZE)
        # cv2.namedWindow(self._camera_window_name_right, cv2.WINDOW_NORMAL)
        cv2.namedWindow(self._camera_window_name_right, cv2.WINDOW_AUTOSIZE)
        # cv2.namedWindow(self._disparity_window_name, cv2.WINDOW_NORMAL)
        cv2.namedWindow(self._disparity_window_name, cv2.WINDOW_AUTOSIZE)
    ...
    def run(self):
        ...
        while not self._stop_flag:
            ...
            with self._cv:
                ...
                cv2.imshow(self._disparity_window_name, cv2.applyColorMap(disp_prediction[0].astype(np.uint8),cv2.COLORMAP_HOT))
                
                self._queue.append(disp_prediction[0].astype(np.uint8))
                print("in_queue\n")
                self._cv.notifyAll()

                cv2.waitKey(1)
                # cv2.imwrite("/home/lipai/self_adapt/Real-time-self-adaptive-deep-stereo/disp/disp_"+str(it).zfill(10)+".png", cv2.applyColorMap(disp_prediction[0].astype(np.uint8),cv2.COLORMAP_HOT))
                it+=1
