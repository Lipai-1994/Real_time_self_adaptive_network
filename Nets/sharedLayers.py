...
INITIALIZER_CONV = tf.initializers.glorot_uniform()
...
def EA(input_, input_1, in_channels, key_channels, head_count, value_channels, name='EA', wName='kqw'):
    # with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    n, h, w, c = input_.get_shape().as_list()
    print("first shape:")
    print(input_.get_shape())

    W = tf.get_variable(wName, [1, 1, c, key_channels], initializer=INITIALIZER_CONV)
    keys = tf.nn.conv2d(input_, W, strides=[1, 1, 1, 1], padding="VALID")
    queries = tf.nn.conv2d(input_1, W, strides=[1, 1, 1, 1], padding="VALID")
    values = conv2d(input_1, [1, 1, c, value_channels], name=name+"_values")
    ...
    print("after shape:")
    print(aggregated_values.get_shape())
    aggregated_values = tf.transpose(aggregated_values, perm=[0, 2, 3, 1])

    chann = aggregated_values.get_shape()[-1]
    reprojected_value = conv2d(aggregated_values, [1, 1, chann, in_channels], name=name+"_reproj") 
