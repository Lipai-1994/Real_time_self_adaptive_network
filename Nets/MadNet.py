...
class MadNet(Stereo_net.StereoNet):
    ...
    def _stereo_context_net(self, input, disp):
        ...
        # self._add_to_layers(names[-1], sharedLayers.EA(input_layer, input_layer, in_channels, 
        #     10, 3, 10, name='EA2'))


        # final_disp = disp + sharedLayers.EA(self._get_layer_as_input(names[-1]), 1, 3, 3, 3)
    ...
    def _pyramid_features(self, input_batch, scope='pyramid', reuse=False, layer_prefix='pyramid'):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            names.append('{}/EA-2'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            in_channels = input_layer.get_shape().as_list()[-1]
            self._add_to_layers(names[-1], sharedLayers.EA(input_layer, input_layer, in_channels, 
                3, 3, 3, name='EA-2', wName='kqw-2'))

            # conv5
            names.append('{}/conv5'.format(layer_prefix))
            # input_layer = self._get_layer_as_input(names[-2])
            input_layer = self._get_layer_as_input('{}/conv4'.format(layer_prefix))
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
            ...
            names.append('{}/EA-1'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            in_channels = input_layer.get_shape().as_list()[-1]
            self._add_to_layers(names[-1], sharedLayers.EA(input_layer, input_layer, in_channels, 
                3, 3, 3, name='EA-1', wName='kqw-1'))

            # conv7
            names.append('{}/conv7'.format(layer_prefix))
            # input_layer = self._get_layer_as_input(names[-2])
            input_layer = self._get_layer_as_input('{}/conv6'.format(layer_prefix))
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
            ...
            names.append('{}/EA0'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            in_channels = input_layer.get_shape().as_list()[-1]
            self._add_to_layers(names[-1], sharedLayers.EA(input_layer, input_layer, in_channels, 
                3, 3, 3, name='EA0', wName='kqw0'))

            # conv9
            names.append('{}/conv9'.format(layer_prefix))
            # input_layer = self._get_layer_as_input(names[-2])
            input_layer = self._get_layer_as_input('{}/conv8'.format(layer_prefix))
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
            ...
            names.append('{}/EA1'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            in_channels = input_layer.get_shape().as_list()[-1]
            self._add_to_layers(names[-1], sharedLayers.EA(input_layer, input_layer, in_channels, 
                3, 3, 3, name='EA1', wName='kqw1'))

            # conv11
            names.append('{}/conv11'.format(layer_prefix))
            # input_layer = self._get_layer_as_input(names[-2])
            input_layer = self._get_layer_as_input('{}/conv10'.format(layer_prefix))
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
            ...
            names.append('{}/EA2'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            in_channels = input_layer.get_shape().as_list()[-1]
            self._add_to_layers(names[-1], sharedLayers.EA(input_layer, input_layer, in_channels, 
                3, 3, 3, name='EA2', wName='kqw2'))
    ...
    def _build_network(self, args):
        ...
        with tf.variable_scope("G6"):
            with tf.variable_scope("unary-6"):
                ...
                # left_0_sample_6 = self._get_layer_as_input('left/EA2')
                right_0_sample_6 = self._get_layer_as_input('right/conv12')
                # right_0_sample_6 = self._get_layer_as_input('right/EA2')

                # in_channels = left_0_sample_6.get_shape()[-1]
                # left_0_sample_6 = sharedLayers.EA(left_0_sample_6, right_0_sample_6, in_channels, 6, 3, 6, name='EA_LR', wName="kwq_lr")
                # right_0_sample_6 = sharedLayers.EA(right_0_sample_6, left_0_sample_6, in_channels, 6, 3, 6, name='EA_RL', wName="kwq_rl")
        ...
        with tf.variable_scope("G5"):
            with tf.variable_scope("unary-5"):
                ...
                # left_0_sample_5 = self._get_layer_as_input('left/EA1')
                if args['warping']:
                    ...
                    # right_0_sample_5 = self._linear_warping(self._get_layer_as_input('right/EA1'), self._build_indeces(tf.concat([u5, tf.zeros_like(u5)], -1)))
                else:
                    right_0_sample_5 = self._get_layer_as_input('right/conv10')
                    # right_0_sample_5 = self._get_layer_as_input('right/EA1')
        ...
        with tf.variable_scope('G4'):
            with tf.variable_scope('unary-4'):
                ...
                # left_0_sample_4 = self._get_layer_as_input('left/EA0')
                if args['warping']:
                    ...
                    # right_0_sample_4 = self._linear_warping(self._get_layer_as_input('right/EA0'), self._build_indeces(tf.concat([u4, tf.zeros_like(u4)], -1)))
                else:
                    right_0_sample_4 = self._get_layer_as_input('right/conv8')
                    # right_0_sample_4 = self._get_layer_as_input('right/EA0')
        ...
        with tf.variable_scope('G3'):
            with tf.variable_scope('unary-3'):
                ...
                # left_0_sample_3 = self._get_layer_as_input('left/EA-1')
                if args['warping']:
                    ...
                    # right_0_sample_3 = self._linear_warping(self._get_layer_as_input('right/EA-1'), self._build_indeces(tf.concat([u3, tf.zeros_like(u3)], -1)))
                else:
                    right_0_sample_3 = self._get_layer_as_input('right/conv6')
                    # right_0_sample_3 = self._get_layer_as_input('right/EA-1')
        ...
        with tf.variable_scope('G2'):
            with tf.variable_scope('unary-2'):
                ...
                # left_0_sample_2 = self._get_layer_as_input('left/EA-2')
                if args['warping']:
                    ...
                    # right_0_sample_2 = self._linear_warping(self._get_layer_as_input('right/EA-2'), self._build_indeces(tf.concat([u2, tf.zeros_like(u2)], -1)))
                else:
                    right_0_sample_2 = self._get_layer_as_input('right/conv4')
                    # right_0_sample_2 = self._get_layer_as_input('right/EA-2')
        ...
        # final = self._get_layer_as_input('final_disp')
        # rescaled_prediction = tf.nn.relu(tf.image.resize_images(sharedLayers.EA(final, final, 1, 3, 3, 3), [image_height, image_width]) * -20.)
